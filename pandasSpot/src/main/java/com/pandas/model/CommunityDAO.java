...
// SpotMapper에서 getCommunities 호출
	...
	public Communities updateViews(int comm_idx) 	{
		SqlSession session = sqlSessionFactory.openSession(true);
		int res = session.update("SpotMapper.CommViewsUP", comm_idx);
		System.out.println(res > 0 ? "view 업데이트 성공" : "view 업데이트 실패");
		System.out.println(comm_idx);
		Communities views = session.selectOne("SpotMapper.getCommunities", comm_idx);
		session.close();
		return views;
	}
}
