...
<body>
	...
	<% 
	String tBirth = member.getMem_birthdate();
	String birth = tBirth.substring(0, 4) + "-" + tBirth.substring(4, 6) + "-" + tBirth.substring(6); 
	double sNum = member.getMem_school();
	SchoolDAO sDAO = new SchoolDAO();
	Schools s = sDAO.getSchoolInfo(sNum);
	%>
	<div class="content-container" data-aos="fade">
		<div>
			<h2 class="join-title">Update Your Info!</h2>
			<!-- join : 은유 -->
			<div>
				<div>
					<form action="Update" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
						...
						<input class="join-input input_disable" type="text" name="mem_id" id="mem_id" placeholder="아이디를 입력하세요" value="<%= member.getMem_id() %>" readonly/>  
						<input class="join-input" style="font-Family:arial" type="password" id="mem_pw" name="mem_pw" placeholder="비밀번호를 입력하세요" oninput="changeFontFamily()" value="<%= member.getMem_pw() %>"> 
						<input class="join-input" style="font-Family:arial" type="password" id="pw_confirm" name="pw_confirm" placeholder="비밀번호 확인" oninput="validatePasswords()" value="<%= member.getMem_pw() %>">
						...
						<input class="join-input" type="date" id="mem_birthdate" name="mem_birthdate" value="<%= birth %>"> 
						<div class="message" id="birth_message"></div>
						<input class="join-input" type="text" id="mem_school" name="mem_school" placeholder="학교명을 입력하세요(예: 한국고등학교 -> 한국)" value="<%= s.getSchool_name() + "-" + s.getSchool_region() %>"> 
						...
						<input class="join-input join-input-btn btn" type="submit" value="정보 수정">
					</form>
				</div>
			</div>
		</div>
	...
	<script>
		...
		document.getElementById('mem_profile_img').addEventListener('change',
				handleImageChange);
		...
		function validateForm() {
			...
			const img = document.getElementById("mem_profile_img").value;
			...
			if (id === "") {
				const errorMessage = document
						.getElementById("id_check_message");
				errorMessage.style.display = "block";
