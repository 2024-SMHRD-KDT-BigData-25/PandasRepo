...
<div class="pages_info" id="pages_info"><%= length %></div>
	...
	<div id="table_footer"><div id="page_list"></div><button class="btn board_btn" id="to_write_btn" onclick="location.href='questionpost.jsp'" style="float: right">글작성</button></div>
	</div>
</div>
  ...
  <script>
	...
	//비동기 통신 시 사용하는 데이터 형식 : json {key:value,key:value} / xml
		...
		for(var board of data){
	      ...
	      html += "<td class='text-black board_content_line'>"+board.q_idx+"</td>"
	      html += "<td class='text-black board_content_line td_title'><a class='table_a_tags' href='question_view.jsp?idx="+board.q_idx+"'>"+board.q_title+"</a></td>"
	      html += "<td class='text-black board_content_line'><a>"+board.mem_id+"</a></td>"
	      html += "<td class='text-black board_content_line'>"+board.q_workbook+"</td>"
