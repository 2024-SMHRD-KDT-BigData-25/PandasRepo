  ...
  <script>
        ...
        fileUpload.addEventListener('change', function(event) {
            ...
            if (file && file.type.startsWith('image/')) {
                ...
                // 파일을 읽은 후 미리보기 이미지로 설정
                reader.onload = function(e) {
                    imagePreview.src = e.target.result; // 이미지 소스 설정
                    imagePreview.style.display = 'block'; // 이미지 미리보기 표시
                };

                reader.readAsDataURL(file); // 파일을 읽어 DataURL 형식으로 변환
            } else {
                imagePreview.style.display = 'none'; // 이미지가 아니면 미리보기를 숨김
